...
## Slack 운영 연결 (2026-09-07)

- 워크스페이스: AXONE (`T0C05EE1UCC`, `theaxone.slack.com`).
- 수신 채널: 비공개 `상담접수` (`C0C007H844E`). 생성 시 담당자 본인만 참여하며 추가 담당자는 Slack에서 초대합니다.
- 앱: `AXONE Consultations` (`A0C042R1DPB`), `incoming-webhook` 권한만 부여.
- 웹후크는 Vercel Production의 민감 환경 변수 `SLACK_WEBHOOK_URL`로 저장합니다. 공개 저장소에는 URL을 기록하지 않습니다.
- 개인 ‘이원희’ 워크스페이스의 기존 AXONE 앱과 별개입니다. Gmail 자동 수집은 아직 활성화하지 않았습니다.
