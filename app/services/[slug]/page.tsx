...
export default async function ServicePage({ params }: Props) {
  ...
  return <>
    ...
    <article className="container seo-article">
      ...
      {price && <section className="seo-callout"><h2>비용과 제공 범위는 어떻게 되나요?</h2><p className="seo-price">{priceLabel(price)}</p><p>{price.unit} · {price.scope}</p><p>상담에서 확인할 사항: {price.confirm}.</p>{price.note && <p>{price.note}</p>}<p>최종 제공 범위와 비용은 착수 전에 견적서로 확정합니다.</p><Link href={`/pricing?items=${price.id}`}>전체 가격과 견적 계산 보기 →</Link></section>}
      ...
      <div className="seo-actions"><Link href={contactForService(service.id)} className="btn btn--primary">{service.title} 상담하기</Link><Link href="/pricing" className="btn btn--ghost">가격 확인하기</Link></div>
    </article>
  </>;
}
